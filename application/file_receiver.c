...
// Server socket to receive client connect and process file transfer
static void *server_socket(void* argv) {
	...
	while (1) {
		printf("[RECEIVER] TCP Server is setup : listening with port <%d> \n", tcp_port);
		connfd = accept(sockfd, (struct sockaddr *)&client_addr, &len);
		assert(connfd >= 0);

		printf("[RECEIVER] TCP client connection is accepted with port <%d>\n", tcp_port);
		...
		while (1) {
			...
			if ( socket_recv_size == 0 ) {
				// socket is ended
				printf("[ERROR] Socket is ended, errno = %s \n", strerror(errno));
 				break;
			...
			if ( socket_recv_size != header_h.pld_len ) {
				...
				// printf("[DBBUG] Server socket <%d> : FILE_TRANSFER_CMD_START paylod read %ld bytes from client\n ", socket_server_id, socket_recv_size);
			}
			...
			printf("[RECEIVER] Server socket <%d> : file path = \"%s\"\n", socket_server_id, tmp_file_path);

			// 2 FILE_TRANSFER_CMD_DATA
			rx_recv_bytes = 0;
			fp = open_file(output_file_path);
			if ( !fp ) {
				printf("[ERROR] Server socket <%d> : file open failed, file path = \"%s\" errno = %s \n", socket_server_id, tmp_file_path, strerror(errno));
				assert(0);
			}
			...
			while (1) {
				...
				if ( header_h.msg_type == FILE_TRANSFER_CMD_END ) {
					printf("[RECEIVER] Server socket <%d> : file \"%s\" transmission is completed\n", socket_server_id, tmp_file_path);
					break; 
				}
				...
				if ( socket_recv_size != header_h.pld_len ) {
					...
					printf("[DBBUG] Server socket <%d> : recv %ld bytes from client, seq num %u\n", socket_server_id, socket_recv_size, header_h.seq_num);
				}
			...
			// File transfer complete for one file
			printf("[RECEIVER] Server socket <%d> : file transfer complete, file path = \"%s\", total received bytes = %ld \n", socket_server_id, tmp_file_path, rx_recv_bytes);
			fclose(fp);
		}
...
// Recursively create dir if not exists
static void recursive_create_dir(char *dir_path) {
...
// File open wrapper in case file path folder doesn't exist
static FILE *open_file(char *file_path) {
	FILE *fp;
	char tmp_folder_path[MAX_FILE_PATH_LEN];
...
// Convert network bytes order to host bytes order for FILE_TRANSFER_HEADER
static FILE_TRANSFER_HEADER network_to_host_byte_order(FILE_TRANSFER_HEADER header_n) {
